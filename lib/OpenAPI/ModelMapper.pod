...
This class generates DTO classes for each item in the C<components/schemas>
subsection of an OpenAPI specification. Each class is generated for a given
object system. Currently L<Moo> is only supported. Each class has the prefix
"DTO". This prefix isn't the root prefix (C<My::App> for example) of an
application that has used this class to get initialized.
